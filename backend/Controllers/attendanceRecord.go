...
func checkAbsence(courseID string, studentID string, absenceDate string) bool{
    ...
    deleteStmt, deleteerr := db.Prepare("DELETE FROM absence WHERE CourseID=? AND StudentID=? AND AttendanceDate LIKE ?")
    if deleteerr != nil{
        fmt.Println(deleteerr)
    }

    row:= db.QueryRow("SELECT AttendanceDate FROM absence WHERE CourseID=? AND StudentID=? AND AttendanceDate LIKE ?", courseID, studentID, absenceDate+"%")
