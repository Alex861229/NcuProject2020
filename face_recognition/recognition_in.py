...
def Record():
  ...
  print(UserID)
  # UserID = facerecord.queue[0]
  sql = "INSERT INTO facerecord (UserID, CourseID, status) VALUES (%s,%s,%s)"
  records = [(UserID, CourseID, 'in')]
  ...
  if(dist != []):
    ...
    if(cd_sorted[0][1] < 0.5):
      ...
      if rec_name not in list(facerecord.queue) :
        print('before put：', list(facerecord.queue))
        facerecord.put(rec_name)
        print('after put：', list(facerecord.queue))
        # 建立一個子執行緒
